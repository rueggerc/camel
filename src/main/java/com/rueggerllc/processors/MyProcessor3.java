...
public class MyProcessor3 implements Processor {
	
	private static final Logger logger = Logger.getLogger(MyProcessor3.class);

	public void process(Exchange exchange) throws Exception {
		try {
			logger.info("MyProcessor3 BEGIN");
			showProperties(exchange);
			Object body = exchange.getIn().getBody();
			logger.info("body=" + body);
			logger.info("body class=" + body.getClass().getName());
			exchange.getIn().setBody(new MyBean());
		} catch (Exception e) {
